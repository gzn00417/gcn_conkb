...
def parse_args():
    ...
    # network arguments
    args.add_argument(
        "-data", "--data", default="./data/WN18RR/", help="data directory"
    )
    args.add_argument(
        "-e_g", "--epochs_gat", type=int, default=3600, help="Number of epochs"
    )
    args.add_argument(
        "-e_c", "--epochs_conv", type=int, default=200, help="Number of epochs"
    )
    args.add_argument(
        "-w_gat",
        "--weight_decay_gat",
    ...
    # arguments for GAT
    args.add_argument(
        "-b_gat", "--batch_size_gat", type=int, default=86835, help="Batch size for GAT"
    )
    args.add_argument(
        "-neg_s_gat",
    ...
    args.add_argument(
        "-margin", "--margin", type=float, default=5, help="Margin used in hinge loss"
    )
...
def get_data(args):
    train_data, validation_data, test_data, entity2id, relation2id, headTailSelector, unique_entities_train = build_data(
        path=args.data, is_unweighted=False, directed=True
    )

    if args.pretrained_emb:  # 预训练
        entity_embeddings, relation_embeddings = init_embeddings(
            os.path.join(args.data, "entity2vec.txt"),  # 已有实体向量初始值
            os.path.join(args.data, "relation2vec.txt"),  # 已有关系向量初始值
        )
        print("Initialized relations and entities from TransE")
    else:  # 随机生成
        ...
        print("Initialized relations and entities RANDOMLY")
    ...
    corpus = Corpus(
        ...
        headTailSelector=headTailSelector,
        batch_size=args.batch_size_gat,
        valid_to_invalid_samples_ratio=args.valid_invalid_ratio_gat,
        unique_entities_train=unique_entities_train,
        get_2hop=args.get_2hop,
    )  # 生成语料对象
...
def train_gat(args):
    ...
    print(
        "\nModel type -> GAT layer with {} heads used , Initital Embeddings training".format(
            args.nheads_GAT[0]
        )
    )
    ...
    if CUDA:
        current_batch_2hop_indices = current_batch_2hop_indices.cuda()
    ...
    for epoch in range(args.epochs_gat):
        ...
        if len(Corpus_.train_indices) % args.batch_size_gat == 0:
            ...
            num_iters_per_epoch = (
                len(Corpus_.train_indices) // args.batch_size_gat
            ) + 1
        ...
        for iters in range(num_iters_per_epoch):
            ...
            print(
                "Iteration-> {0}  , Iteration_time-> {1:.4f} , Iteration_loss {2:.4f}".format(
        ...
        scheduler.step()
        print(
            "Epoch {} , average loss {} , epoch_time {}".format(
                epoch, sum(epoch_loss) / len(epoch_loss), time.time() - start_time
            )
        )
        epoch_losses.append(sum(epoch_loss) / len(epoch_loss))

    save_model(model_gat, args.data, args.epochs_gat - 1, args.output_folder)
...
def train_conv(args):
    ...
    model_gat.load_state_dict(
        torch.load("{}/trained_{}.pth".format(args.output_folder, args.epochs_gat - 1)),
        strict=False,
    )
    ...
    for epoch in range(args.epochs_conv):
        ...
        if len(Corpus_.train_indices) % args.batch_size_conv == 0:
            ...
            num_iters_per_epoch = (
                len(Corpus_.train_indices) // args.batch_size_conv
            ) + 1
    ...
    save_model(model_conv, args.data, args.epochs_gat - 1, args.output_folder + "conv/")
...
if __name__ == "__main__":
    ...
    Corpus_, entity_embeddings, relation_embeddings = get_data(args)
    if args.get_2hop:
    ...
    train_gat(args)  # Train Encoder
    train_conv(args)  # Train Decoder
    evaluate_conv(args, Corpus_.unique_entities_train)  # Evaluate
